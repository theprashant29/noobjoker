 ...
 *config.prefix
 * For example, If you set the access to '/proxy/' and pass in this chunk of html:
...
function urlPrefixer(config) {
    var re_abs_url = /("|'|=|url\(\s*)(https?:)/ig, // "http:, 'http:, =http:, or url( http:, also matches https versions
        ...
        re_proxied_xmlns = new RegExp('(xmlns(:[a-z]+)?=")' + access, 'ig'),
        re_proxied_doctype = new RegExp('(<!DOCTYPE[^>]+")' + access, 'i');
    ...
    function createStream(uri) {
        ...
        return new Transform({
            ...
            transform: function (chunk, encoding, next) {
                ...
                chunk = rewriteUrls(chunk, uri, access);
            ...
            flush: function(done) {
                ...
                if (chunk_remainder) {
                    this.push(rewriteUrls(chunk_remainder, uri, access));
                    chunk_remainder = undefined;
                }
                done();
            }
    ...
    function prefixUrls(data) {
        if (contentTypes.shouldProcess(config, data)) {
            ...
            debug('prefixing all urls with %s', access);
